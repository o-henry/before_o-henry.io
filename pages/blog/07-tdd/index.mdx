...
프론트엔드 에서는 UI 컴포넌트가 페이지를 렌더링하기 전에 API 요청을 통해 데이터를 가져오는 일이 흔하다.
위의 과정을 테스트 하려면 일반적으로 실제 API 요청을 하는 과정을 테스팅 하게 될텐데, 이는 좋은 방법이 아니다.
테스트 시간이 많이 소요될 수 있고, 같은 요청에 다른 데이터가 올수도있는 등의 예측하기 어려운 문제가 일어날 수 있기 때문이다.
